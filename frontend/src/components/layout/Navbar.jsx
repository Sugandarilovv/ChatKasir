...
export default function Navbar({ onMenuClick, isMenuOpen }) {
  ...
  return (
    <header
      className="h-16 flex items-center justify-between px-4 sm:px-6 border-b sticky top-0 z-50 transition-colors duration-300"
      style={{
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3">
        <button
          onClick={onMenuClick}
          className="md:hidden relative z-50 p-2 -ml-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          title="Toggle Menu"
        >
        ...
        <img 
          src={logoImg} 
          alt="Logo ChatKasir" 
          className="w-8 h-8 sm:w-9 sm:h-9 rounded-md shadow-sm object-contain" 
        />
        {/* PERBAIKAN: class "hidden" dihapus agar tulisan ChatKasir selalu tampil */}
        <span className={`font-extrabold text-base sm:text-lg tracking-tight ${isDark ? 'text-white' : 'text-gray-800'}`}>
          ChatKasir
        </span>
      </div>
