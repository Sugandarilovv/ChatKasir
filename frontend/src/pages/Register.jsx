...
export default function Register() {
  ...
  function onSubmit(data) { handleRegister(data.nama, data.email, data.password) }
    ...
    <div className="min-h-screen flex bg-white font-sans">
      <div className="hidden lg:flex flex-col w-125 shrink-0 relative bg-green-950 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col h-full p-12">
          ...
          <div className="space-y-4">
            {FITUR.map((f) => (
              <div key={f.title} className="flex items-start gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm transition-colors">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-gray-50/50 overflow-y-auto">
        <div className="w-full max-w-md animate-fade-up py-4">
          ...
          <div className="mb-6">
            <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight mb-2">Buat Akun Baru</h2>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="text" placeholder="Masukkan nama Anda"
                className={`w-full px-4 py-3 rounded-xl text-sm outline-none border ${errors.nama ? 'border-red-300 bg-red-50 focus:border-red-500 focus:ring-4' : 'border-gray-200 bg-white focus:border-green-500 focus:ring-4 focus:ring-green-500/10'}`}
            ...
            <div className="space-y-1.5">
              ...
              <input type="email" placeholder="contoh@umkm.com"
                className={`w-full px-4 py-3 rounded-xl text-sm outline-none border ${errors.email ? 'border-red-300 bg-red-50 focus:border-red-500 focus:ring-4' : 'border-gray-200 bg-white focus:border-green-500 focus:ring-4 focus:ring-green-500/10'}`}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} placeholder="Minimal 8 karakter"
                  className={`w-full pl-4 pr-12 py-3 rounded-xl text-sm outline-none border [&::-ms-reveal]:hidden [&::-ms-clear]:hidden ${errors.password ? 'border-red-300 bg-red-50 focus:border-red-500 focus:ring-4' : 'border-gray-200 bg-white focus:border-green-500 focus:ring-4 focus:ring-green-500/10'}`}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input type={showPass2 ? 'text' : 'password'} placeholder="Ketik ulang password"
                  className={`w-full pl-4 pr-12 py-3 rounded-xl text-sm outline-none border [&::-ms-reveal]:hidden [&::-ms-clear]:hidden ${errors.konfirmasi ? 'border-red-300 bg-red-50 focus:border-red-500 focus:ring-4' : 'border-gray-200 bg-white focus:border-green-500 focus:ring-4 focus:ring-green-500/10'}`}
                  {...register('konfirmasi', { required: 'Wajib diisi', validate: (v) => v === watch('password') || 'Password tidak cocok' })} />
                <button type="button" onClick={() => setShowPass2(!showPass2)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-gray-600">
